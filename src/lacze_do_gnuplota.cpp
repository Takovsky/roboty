...
LaczeDoGNUPlota::LaczeDoGNUPlota()
{
  ...
  UstawZakresX(-200, 200);
  UstawZakresY(-200, 200);
...
void LaczeDoGNUPlota::BudujPreambule_3D(std::string &Preambula) const
{
  ...
  if (_PokazOs_OY)
    Preambula += "set yzeroaxis lt 0 lw 1\n";
  if (_PokazOs_OZ)
    Preambula += "set zzeroaxis lt 0 lw 1\n";
...
void LaczeDoGNUPlota::ZmienSzerokoscRysowania(const char *nazwa, int szerokosc)
{
  for (auto pd = _InfoPlikow.begin(); pd != _InfoPlikow.end(); pd++)
  {
    if (strstr(nazwa, pd->WezNazwePliku().c_str()))
    {
      pd->WezSzerokosc() = szerokosc;
      break;
    }
  }
}

void LaczeDoGNUPlota::UsunNazwe(const char *nazwa)
{
  for (auto pd = _InfoPlikow.begin(); pd != _InfoPlikow.end(); pd++)
  {
    if (strstr(nazwa, pd->WezNazwePliku().c_str()))
    {
      _InfoPlikow.erase(--pd, ++pd);
      break;
    }
  }
}
